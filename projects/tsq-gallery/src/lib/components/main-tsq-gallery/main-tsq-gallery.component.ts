...
export class TSqGalleryComponent {
  ...
  /** Override the class of the container for each file of the Preview. The class should either have `::ng-deep` (not recommended) or be global. */
  ...
  // tslint:disable-next-line: max-line-length
  /** Override the inner content for pdf type files on the Preview. This can be used if you want to hide the file. Check `TSqGalleryListItemTemplateRefContext` for variables access. */
  ...
  /** Toggle display the Preview or a single access action buttom. Override `imagePreviewTemplate` to edit it in case of `false`. */
